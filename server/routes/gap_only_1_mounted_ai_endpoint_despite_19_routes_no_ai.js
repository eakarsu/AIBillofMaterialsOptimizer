...
// Feature: Only 1 mounted AI endpoint despite 19 routes; no AI BOM cost optimization
  ...
  try {
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({
        ...
        feature: 'gap_only_1_mounted_ai_endpoint_despite_19_routes_no_ai',
      });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${ 'Only 1 mounted AI endpoint despite 19 routes; no AI BOM cost optimization' }\n\nReturn JSON: {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const system = `You are an expert assistant for the feature: 'Only 1 mounted AI endpoint despite 19 routes; no AI BOM cost optimization'. Respond with strict JSON.`;
    ...
    const out = { feature: 'gap_only_1_mounted_ai_endpoint_despite_19_routes_no_ai', model: aiResp?.model, parsed, raw: content };
    if (dbPool) {
      try {
        await dbPool.query(
          'INSERT INTO gap_features (feature_key, user_id, input_payload, output_payload) VALUES ($1, $2, $3, $4)',
          ['gap_only_1_mounted_ai_endpoint_despite_19_routes_no_ai', req.user?.id || null, JSON.stringify(payload), JSON.stringify(out)]
        );
...
router.get('/health', (req, res) => res.json({ feature: 'gap_only_1_mounted_ai_endpoint_despite_19_routes_no_ai', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
...
router.get('/history', async (req, res) => {
  ...
  try {
    ...
    const r = await dbPool.query(
      ...
      ['gap_only_1_mounted_ai_endpoint_despite_19_routes_no_ai']
    );
