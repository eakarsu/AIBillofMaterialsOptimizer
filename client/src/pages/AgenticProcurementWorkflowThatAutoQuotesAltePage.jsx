...
// Feature: Agentic procurement workflow that auto-quotes alternative parts and chases supplier confirmations
import React, { useState } from 'react';
import api from '../services/api';

export default function AgenticProcurementWorkflowThatAutoQuotesAltePage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-agentic-procurement-workflow-that-auto-quotes-alte/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic procurement workflow that auto-quotes alternative parts and chases supplier confirmations</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-procurement-workflow-that-auto-quotes-alte/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
