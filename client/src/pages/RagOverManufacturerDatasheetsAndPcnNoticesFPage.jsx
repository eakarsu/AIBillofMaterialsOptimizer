...
// Feature: RAG over manufacturer datasheets and PCN notices for compliance Q&A
import React, { useState } from 'react';
import api from '../services/api';

export default function RagOverManufacturerDatasheetsAndPcnNoticesFPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-rag-over-manufacturer-datasheets-and-pcn-notices-f/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>RAG over manufacturer datasheets and PCN notices for compliance Q&A</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-rag-over-manufacturer-datasheets-and-pcn-notices-f/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
