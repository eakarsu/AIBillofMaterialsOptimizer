...
export default function NoOutboundEdiForSupplierPurchaseOrdersWebhoPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-no-outbound-edi-for-supplier-purchase-orders-webho/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No outbound EDI for supplier purchase orders (webhooks exist but no PO workflow)</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-outbound-edi-for-supplier-purchase-orders-webho/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
